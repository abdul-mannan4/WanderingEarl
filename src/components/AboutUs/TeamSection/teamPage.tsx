...
export default function TeamPage() {
  return (
    <div className="w-full pt-8 sm:pt-[52px] lg:pt-[100px] flex flex-col gap-[30px] sm:gap-[40px] md:gap-[52px] lg:gap-[120px]">
      {TeamData.map((member, index) => {
        ...
        return (
          <div
            key={member.name}
            className={`w-full flex flex-col ${isReversed ? "lg:flex-row-reverse" : "lg:flex-row"
              } items-stretch lg:items-start xl:items-center gap-4 lg:gap-[40px] xl:gap-[64px]`}
          >
            {/* Image - Responsive size (40% on lg, 44% on xl, max-622px on 2xl) to prevent text squishing */}
            <div className="relative w-full max-w-full lg:w-[40%] xl:w-[44%] 2xl:max-w-[622px] aspect-square rounded-[16px] sm:rounded-[24px] overflow-hidden shadow-lg flex-shrink-0">
              <Image
                src={member.image}
                alt={member.name}
                fill
            ...
            <div className="flex flex-col gap-0.5 lg:gap-[32px] xl:gap-[48px] font-inter text-text-dark text-[15px] sm:text-[16px] xl:text-[18px] leading-relaxed w-full flex-1 min-w-0">
              ...
              <div className="order-2 lg:order-1 flex flex-col gap-0.5 lg:gap-[20px] xl:gap-[24px]">
